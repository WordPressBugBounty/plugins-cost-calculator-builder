<div class="cbb-edit-field-container">
	<div class="ccb-edit-field-header">
		<div class="ccb-edit-field-header-left">
			<span class="ccb-edit-field-title ccb-heading-3 ccb-bold">{{ getFieldTitle(toggleField) || "Switch toggle" }}</span>
		</div>
		<div class="ccb-edit-field-header-right">
			<div class="ccb-save-wrapper">
				...
				<button class="ccb-button success" @click.prevent="save(toggleField, id, index, toggleField.alias)"><?php esc_html_e( 'Done', 'cost-calculator-builder' ); ?></button>
			</div>
		</div>
	</div>
	<div class="ccb-grid-box">
				...
				<div class="col-12">
					<div class="ccb-field-name">
						<div class="ccb-field-name__header">
							...
							<div class="ccb-field-name__id">{{ toggleField.alias }}</div>
						</div>
						<div class="ccb-field-name__body">
							<input type="text" placeholder="<?php esc_attr_e( 'Enter field name', 'cost-calculator-builder' ); ?>" v-model.trim="toggleField.label">
							<input type="text" placeholder="<?php esc_attr_e( 'Enter field description', 'cost-calculator-builder' ); ?>" v-model.trim="toggleField.description">
						</div>
					</div>
				</div>
			</div>
				...
				<div class="col-12">
					<div class="ccb-builder-radio-wrapper">
						...
						<div class="ccb-radio-box">
							<input class="ccb-builder-radio__radio" :id="'ccb-width-25-' + toggleField.alias" type="radio" :name="'width-' + toggleField.alias" value="25" v-model="toggleField.width" @change="document.dispatchEvent(new CustomEvent('ccb_field_width_change', { detail: { alias: id.alias, width: 25 } }))">
							<label class="ccb-builder-radio__option" :for="'ccb-width-25-' + toggleField.alias"><?php esc_html_e( '25', 'cost-calculator-builder' ); ?></label>

							<input class="ccb-builder-radio__radio" :id="'ccb-width-50-' + toggleField.alias" type="radio" :name="'width-' + toggleField.alias" value="50" v-model="toggleField.width" @change="document.dispatchEvent(new CustomEvent('ccb_field_width_change', { detail: { alias: id.alias, width: 50 } }))">
							<label class="ccb-builder-radio__option" :for="'ccb-width-50-' + toggleField.alias"><?php esc_html_e( '50', 'cost-calculator-builder' ); ?></label>

							<input class="ccb-builder-radio__radio" :id="'ccb-width-75-' + toggleField.alias" type="radio" :name="'width-' + toggleField.alias" value="75" v-model="toggleField.width" @change="document.dispatchEvent(new CustomEvent('ccb_field_width_change', { detail: { alias: id.alias, width: 75 } }))">
							<label class="ccb-builder-radio__option" :for="'ccb-width-75-' + toggleField.alias"><?php esc_html_e( '75', 'cost-calculator-builder' ); ?></label>

							<input class="ccb-builder-radio__radio" :id="'ccb-width-100-' + toggleField.alias" type="radio" :name="'width-' + toggleField.alias" value="100" v-model="toggleField.width" @change="document.dispatchEvent(new CustomEvent('ccb_field_width_change', { detail: { alias: id.alias, width: 100 } }))">
							<label class="ccb-builder-radio__option" :for="'ccb-width-100-' + toggleField.alias"><?php esc_html_e( '100', 'cost-calculator-builder' ); ?></label>
						</div>
					</div>
				</div>
			</div>
			<div class="row ccb-p-t-15">
				<div class="col-12 ccb-select-box">
					...
					<div class="ccb-multi-select" tabindex="100" @click.prevent="multiselectShow(event)" :class="[{'disabled': isOptionEmpty}]">
						...
						<span v-else-if="toggleField.hasOwnProperty('default') && toggleField.default.length > 0 && toggleField.default.split(',').length > 3" class="anchor ccb-heading-5 ccb-light ccb-selected">
							{{ toggleField.default.split(',').length }} <?php esc_html_e( 'options selected', 'cost-calculator-builder' ); ?>
						</span>
						<span v-else class="anchor ccb-heading-5 ccb-light-3">
								<?php esc_html_e( 'Select default values', 'cost-calculator-builder' ); ?>
						</span>
						<ul class="items" style="z-index: 88;">
							<li v-if="option.optionText.length > 0 && option.optionValue.length > 0" class="option-item" @click="chooseDefaultValues( optionIndex, option )"
								...
								<input :checked="( toggleField.default.length > 0 && toggleField.default.split(',').includes(option.optionValue + '_' + optionIndex) )"
							...
							<div class="ccb-option" v-for="(option, index) in fieldOptions" :key="index">
								...
								<div class="ccb-option-inner label-input">
									<div class="ccb-input-wrapper">
										...
										<input type="text" v-model="option.optionText" placeholder="<?php esc_attr_e( 'Label', 'cost-calculator-builder' ); ?>">
									</div>
								</div>
								<div class="ccb-option-inner">
									<div class="ccb-input-wrapper">
										...
										<input type="text" min="0" step="1" :name="'option_' + index" @keyup="checkRequired('errorOptionValue' + index)" v-model="option.optionValue" placeholder="<?php esc_attr_e( 'Value', 'cost-calculator-builder' ); ?>">
								...
								<div class="ccb-option-inner hint-input">
									<div class="ccb-input-wrapper">
										...
										<input type="text" v-model="option.optionHint" placeholder="<?php esc_attr_e( 'Hint', 'cost-calculator-builder' ); ?>">
									</div>
								</div>
							</div>
						</draggable>
				...
				<div class="row ccb-p-t-15" v-if="toggleField.styles">
					<div class="col-12">
						<div class="ccb-style-preview ccb-field-style-preview" v-for="style in getToggleStyles" :key="style.value" :class="{'active': toggleField.styles.style === style.value}" @click="toggleField.styles.style = style.value">
							...
							<img :src="style.img[toggleField.styles.box_style]">
						</div>
					</div>
				</div>
				<div class="row ccb-p-t-15">
					<div class="col-12">
						<div class="list-header">
							<div class="ccb-switch">
								<input type="checkbox" v-model="toggleField.apply_style_for_all"/>
								<label></label>
							</div>
							<h6 style="font-size: 14px"><?php esc_html_e( 'Apply this toggle style to all toggle fields in this calculator', 'cost-calculator-builder' ); ?></h6>
						</div>
					</div>
				</div>
			<?php else : ?>
					...
					<div class="calc-styles-pro-container">
						<div class="calc-styles-pro-header">
							...
							<div class="calc-styles-pro-header-right">
								<a href="https://stylemixthemes.com/cost-calculator-plugin/pricing/?utm_source=calcwpadmin&utm_medium=freetoprobutton&utm_campaign=toggle_styles&licenses=1&billing_cycle=annual" target="_blank">
									<?php esc_html_e( 'Get Pro', 'cost-calculator-builder' ); ?>
								</a>
							</div>
						</div>
						<div class="calc-styles-pro-content">
							<img src="<?php echo esc_attr( CALC_URL . '/frontend/dist/img/styles/toggle/switch-pro.gif' ); ?>">
						</div>
					</div>
				</div>
			<?php endif; ?>
		...
		<div class="container" v-show="tab === 'options'">
			<div class="row ccb-p-t-15 switch-row">
				...
				<div class="col-12" v-if="!disableFieldHiddenByDefault(toggleField)">
					<div class="list-header">
						<div class="ccb-switch">
				...
				<div class="col-12" v-if="!disableFieldHiddenByDefault(toggleField)">
					<div class="list-header">
						<div class="ccb-switch">
							<input type="checkbox" v-model="toggleField.calculateHidden"/>
							<label></label>
						</div>
				...
				<div class="col-12">
					<div class="list-header">
						<div class="ccb-switch">
							<input type="checkbox" v-model="toggleField.fieldCurrency"/>
							<label></label>
						</div>
			...
			<div class="row ccb-p-t-15">
				<div class="col-12">
					<div class="ccb-input-wrapper">
						...
						<input type="number" min="0" oninput="validity.valid || (value='');" v-model.trim="toggleField.checkedLength" placeholder="<?php esc_attr_e( 'Set the max number of options allowed to select', 'cost-calculator-builder' ); ?>">
					</div>
				</div>
			</div>
			<div class="row ccb-p-t-15">
				<div class="col-12">
					<div class="ccb-input-wrapper">
						...
						<textarea v-model="toggleField.additionalStyles" placeholder="<?php esc_attr_e( 'Set Additional Classes', 'cost-calculator-builder' ); ?>"></textarea>
					</div>
				</div>
			</div>
		</div>
